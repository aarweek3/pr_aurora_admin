...
. Вкладка "Roles" (Auth Control Panel)
Это то, что мы делаем сейчас (наш RolesTabComponent).

Цель: Диагностика и Тестирование.
Что делает:
Не создает новые роли в БД.
Показывает: "Какие роли есть у меня сейчас?"
Матрица доступа: "Куда мне можно зайти с этими ролями?"
Симуляция (Dev Mode): "А давай притворимся, что у меня есть роль Admin, и проверим, пустит ли меня гард?" (без реального изменения настроек в БД).
...
### 14.1. Тест: Подмена личности (Identity Spoofing)

**Цель:** Проверить способность системы обнаруживать манипуляции с данными пользователя в оперативной памяти клиента.

**Последовательность действий:**

1.  **Запуск теста:** Перейдите в `Auth Control Panel` → вкладка `Simulator`.
2.  **Атака:** В разделе «Рискованная зона» нажмите кнопку **«Подменить UserID»**.
3.  **Первичная проверка (UI):**
    - Убедитесь, что уведомление «UserID подменен в памяти!» успешно отобразилось.
    - Проверьте вкладку `Session` или `Tokens`: `UserID` должен измениться на `Hacker-XXX`, а в списке ролей появиться `Admin`, `Sudo`, `GodMode`.
4.  **Валидация (Consistency Check):**
    - Перейдите во вкладку `Tokens`.
    - Нажмите кнопку **«Проверить консистентность»**.
5.  **Ожидаемый результат (Разоблачение):**
    - Система должна вывести красный баннер с ошибкой.
    - В деталях ошибки должно быть указано расхождение между `Email` (клиент: hacker@blackhat.com vs сервер: ваш реальный email) и `Roles`.
6.  **Финальная проверка (API Truth):**
    - Перейдите во вкладку `Playground` и выполните запрос к `/api/Auth/profile`.
    - Убедитесь, что сервер вернул ваши **реальные данные**, проигнорировав подмену в памяти.

---

**Документ подготовлен:** 29.12.2024
**Версия:** 2.1
**Статус:** Обновлено (Добавлены методики тестирования)
