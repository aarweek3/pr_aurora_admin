...
@Component({
          ...
          <div class="demo-section">
            <nz-card nzTitle="Живое превью" class="demo-card">
              ...
              <div class="search-log" *ngIf="lastQuery()">
                <div class="log-item">
                  <span class="log-label">Событие (onSearch):</span>
          ...
          <div class="integration-section">
            ...
            <nz-card nzTitle="🤖 Prompt для AI (Скопируй меня)" class="ai-card">
              <div class="ai-prompt-box">
                <p class="ai-description">Используйте этот текст для быстрой интеграции поиска через ИИ-помощника:</p>
                <av-help-copy-container 
            ...
            <nz-card nzTitle="Как это работает внутри">
              <div class="logic-grid">
                <div class="logic-item">
                  ...
                  <p>Компонент использует внутренний <code>Subject</code>. При наборе текста событие (onSearch) не генерируется мгновенно. Оно "ждет" паузы в 300мс (настраивается), чтобы не спамить API сервера.</p>
                </div>
                <div class="logic-item">
                  <h4>🔘 Ручной триггер</h4>
                  <p>Нажатие на иконку поиска, кнопку "Найти" или клавишу <b>Enter</b> игнорирует задержку и вызывает поиск немедленно.</p>
                </div>
                <div class="logic-item">
                  <h4>🔄 Двусторонние Signals</h4>
                  <p>Проп <code>[(value)]</code> реализован через Angular Model Signals. Это гарантирует мгновенную синхронизацию данных между родителем и поиском без лишних "Side-effects".</p>
                </div>
                <div class="logic-item">
                  <h4>⏳ Состояние Loading</h4>
                  <p>Если <code>[avLoading]="true"</code>, иконка лупы в поле скрывается, и на её месте появляется анимированный спиннер Aurora. Кнопка при этом не блокируется.</p>
                </div>
              </div>
            </nz-card>
          </div>
  ...
  styles: [
    `
      ...
      .demo-section, .integration-section, .api-section, .logic-section { 
        display: flex; 
      ...
      .demo-card { border-radius: 16px; border: none; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05); }
      .search-demo-box { max-width: 600px; margin-bottom: 24px; }
      ...
      .log-item { display: flex; gap: 12px; align-items: center; }
      .log-label { font-size: 14px; color: #166534; font-weight: 700; }
      .log-value { font-family: 'Fira Code', monospace; font-size: 14px; color: #14532d; }
      ...
      .av-checkbox-label:hover { background: #f1f5f9; }
      ...
      .p-label { min-width: 120px; font-weight: 700; color: #64748b; font-size: 13px; }
      
      .ai-prompt-box { padding: 8px; }
      .ai-description { font-size: 14px; color: #64748b; margin-bottom: 12px; }
      ...
      .logic-item h4 { color: #0f172a; margin-bottom: 10px; font-size: 16px; font-weight: 700; }
      .logic-item p { color: #64748b; font-size: 14px; line-height: 1.6; margin: 0; }
...
export class SearchHelpComponent {
  ...
  inputs = [
    { name: 'value', type: 'model<string>', default: "''", desc: 'Двустороннее реактивное связывание' },
    { name: 'avPlaceholder', type: 'string', default: 'Поиск...', desc: 'Placeholder поля ввода' },
    { name: 'avDebounceTime', type: 'number', default: '300', desc: 'Задержка дебаунса в миллисекундах' },
    { name: 'avLoading', type: 'boolean', default: 'false', desc: 'Включает спиннер Aurora вместо иконки' },
    { name: 'showButton', type: 'boolean', default: 'true', desc: 'Показывать ли кнопку "Найти/Search"' },
    { name: 'avSize', type: 'string', default: 'default', desc: 'Размер: small | default | large | x-large' }
  ];
...
4. Добавьте вызов метода фильтрации через событие (onSearch).
