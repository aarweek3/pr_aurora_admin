...
import {
  ...
  PlatformOfAggregatorLocalizationDto,
} from '../../models/platform-of-aggregator.model';

@Component({
  selector: 'app-platform-of-aggregator-details',
  standalone: true,
  imports: [
    CommonModule,
  ...
  template: `
    <div class="details-container" *ngIf="data">
      ...
      <nz-descriptions
        nzTitle="Техническая информация"
        nzBordered
      ...
      >
        ...
        <nz-descriptions-item nzTitle="Порядок сортировки">{{
          data.sortOrder
        }}</nz-descriptions-item>
        ...
        <nz-descriptions-item nzTitle="Иконка">
          <div class="icon-preview" *ngIf="data.iconPath">
            <img [src]="data.iconPath" alt="platform icon" class="platform-img" />
        ...
        <nz-tab *ngFor="let loc of data.localizations" [nzTitle]="tabTitle">
          <ng-template #tabTitle>
            <span class="tab-label">
              ...
              <span
                *ngIf="loc.languageNativeName || loc.languageName || loc.languageCode; else onlyId"
              >
                {{ loc.languageNativeName || loc.languageName || loc.languageCode }} (ID:
                {{ loc.languageOfAggregatorId }})
              </span>
...
export class PlatformOfAggregatorDetailsComponent {
  @Input() data: PlatformOfAggregatorDetailDto | null = null;
}
