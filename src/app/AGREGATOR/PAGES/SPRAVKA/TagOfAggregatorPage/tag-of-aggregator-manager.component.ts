    ...
    <div class="manager-container">
       <div class="manager-header">
          <div class="header-left">
            <h1>Менеджер Тегов <span class="total">Всего: {{ state.total() }}</span></h1>
            <p class="subtitle">Управление тегами и иерархией агрегатора (SEO Ready)</p>
          </div>
          <div class="header-right">
             <nz-radio-group [ngModel]="viewMode()" (ngModelChange)="setViewMode($event)" nzButtonStyle="solid">
...
export class TagOfAggregatorManagerComponent implements OnInit {
  ...
  get isEditModalVisible(): boolean { return this.state.editModalVisible(); }
  set isEditModalVisible(v: boolean) { if (!v) this.state.closeEditModal(); }
