...
@Component({
  ...
  template: `
    <nz-modal
      ...
      [nzBodyStyle]="(state.viewModalMaximized$ | async) ? { height: 'calc(100vh - 108px)' } : { height: 'auto' }"
    >
      ...
      <ng-template #modalTitle>
        <div class="modal-header-custom">
          <div class="header-left">
            ...
            <span>Просмотр программы: <strong>{{ (state.viewItem$ | async)?.canonicalName }}</strong></span>
          </div>
          <div class="header-right">
            <button nz-button nzType="text" (click)="toggleMaximize()" class="maximize-btn">
              <i nz-icon [nzType]="(state.viewModalMaximized$ | async) ? 'fullscreen-exit' : 'fullscreen'"></i>
            </button>
          </div>
        </div>
      </ng-template>
