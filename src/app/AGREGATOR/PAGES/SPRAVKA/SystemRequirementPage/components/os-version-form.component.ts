...
import { PlatformOsVersionDto, PlatformOsVersionLocalizationDto } from '../models/system-requirement.model';
    ...
    >
          ...
          <div class="info-grid">
            <div class="info-row">
              ...
              <div class="info-value">
                <input *ngIf="state.osModalMode() !== 'view'; else viewCode" 
                       nz-input nzSize="small" class="inline-input code-box"
                       [ngModel]="state.osEditingItem()?.systemCode" 
                       (ngModelChange)="updateItem({ systemCode: $event })" />
                <ng-template #viewCode><span class="code-tag">{{ state.osEditingItem()?.systemCode }}</span></ng-template>
              </div>
              
              <div class="info-label">Платформа</div>
              <div class="info-value">
                <nz-select *ngIf="state.osModalMode() !== 'view'; else viewPlatform"
                          nzSize="small" style="width: 100%"
              ...
              <div class="info-value">
                <nz-switch *ngIf="state.osModalMode() !== 'view'; else viewStatus"
                          nzSize="small"
                          [ngModel]="state.osEditingItem()?.isActive ?? true"
                          (ngModelChange)="updateItem({ isActive: $event })"></nz-switch>
            ...
            <div class="info-row">
              ...
              <div class="info-value">
                <input *ngIf="state.osModalMode() !== 'view'; else viewSort" 
                       type="number" nz-input nzSize="small" class="inline-input"
                  ...
                  <div class="tab-content">
                    <div class="field-row">
                      ...
                      <div class="field-control">
                        <input *ngIf="state.osModalMode() !== 'view'; else viewName" 
                               nz-input [ngModel]="getLocValue(lang.id, 'name')"
                               (ngModelChange)="updateLoc(lang.id, { name: $event })"
                               placeholder="Введите название на языке {{ lang.nativeTitle }}" />
                        <ng-template #viewName><span class="name-text">{{ getLocValue(lang.id, 'name') }}</span></ng-template>
                      </div>
                    </div>
                    ...
                    <div class="field-row mt-12">
                       ...
                       <div class="field-control">
                          <textarea *ngIf="state.osModalMode() !== 'view'; else viewDesc"
                                    nz-input [nzAutosize]="{ minRows: 2 }"
                                    [ngModel]="getLocValue(lang.id, 'description')"
                                    (ngModelChange)="updateLoc(lang.id, { description: $event })"
                                    placeholder="Введите описание (опционально)"></textarea>
                          <ng-template #viewDesc>
                             <div class="text-muted italic">{{ getLocValue(lang.id, 'description') || 'Описание отсутствует.' }}</div>
                          </ng-template>
                       </div>
                    </div>
                  </div>
      ...
      <ng-template #modalTitle>
        <div class="modal-header-custom">
          <i nz-icon [nzType]="state.osModalMode() === 'view' ? 'eye' : 'edit'" class="header-icon"></i>
