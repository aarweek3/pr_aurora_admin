...
import { DeveloperOfAggregatorDetail, DeveloperOfAggregatorLocalization } from '../../models/developer-of-aggregator.model';
 ...
 * 
 * Компонент для отображения детальной информации о разработчике (вендоре).
 * Используется в модальном окне просмотра.
 */
@Component({
  selector: 'app-developer-of-aggregator-details',
  standalone: true,
  imports: [
    CommonModule, 
  ...
  template: `
    <div class="details-container" *ngIf="data">
      ...
      <nz-descriptions nzTitle="Техническая информация" nzBordered [nzColumn]="{ xxl: 3, xl: 3, lg: 3, md: 3, sm: 2, xs: 1 }">
        <nz-descriptions-item nzTitle="Системный код">
        ...
        <nz-descriptions-item nzTitle="Сайт">
          <a *ngIf="data.website" [href]="data.website" target="_blank">{{ data.website }}</a>
          <span *ngIf="!data.website">—</span>
        </nz-descriptions-item>

        <nz-descriptions-item nzTitle="Иконка">
          <div class="icon-preview" *ngIf="data.iconPath">
            <img [src]="imgService.getAssetUrl(data.iconPath)" alt="developer icon" class="developer-img" />
            <span class="path-text">{{ data.iconPath }}</span>
          </div>
          <span *ngIf="!data.iconPath">—</span>
        </nz-descriptions-item>
  ...
  styles: [`
    ...
    .developer-img { width: 32px; height: 32px; object-fit: contain; }
    .path-text { color: #8c8c8c; font-size: 12px; font-family: monospace; }
...
export class DeveloperOfAggregatorDetailsComponent {
  public imgService = inject(ImageServiceUniversal);
  @Input() data: DeveloperOfAggregatorDetail | null = null;
}
