...
import { TinymceBridgeService } from '@core/services/bridges/tinymce-bridge.service';
  ...
  implements OnInit, AfterViewInit, OnDestroy, ControlValueAccessor
{
  ...
  ngAfterViewInit() {
    // Инициализируем редактор вне зоны Angular, используя staggered-delay и requestIdleCallback
    this.ngZone.runOutsideAngular(() => {
      const scheduleInit = () => {
        if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
          window.requestIdleCallback(() => this.initEditor(), { timeout: 1000 });
        } else {
          setTimeout(() => this.initEditor(), 200);
        }
      };

      // Случайный сдвиг (stagger) от 50 до 250 мс, чтобы избежать одновременного запуска нескольких редакторов
      const staggerDelay = Math.random() * 200 + 50;
      setTimeout(scheduleInit, staggerDelay);
    });
  }
