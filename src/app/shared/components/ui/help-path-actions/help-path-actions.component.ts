...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NzButtonModule, NzIconModule, NzToolTipModule, ClipboardModule],
  template: `
    <div class="path-actions">
      <button
        ...
        nz-tooltip
        nzTooltipTitle="Копировать путь"
      >
      ...
      <button
        ...
        nz-tooltip
        nzTooltipTitle="Открыть в редакторе"
      >
...
export class HelpPathActionsComponent {
  ...
  openInEditor(path: string): void {
    ...
    this.iconLabService.openFile(path).subscribe({
      ...
      error: (err) =>
        this.message.error('Не удалось открыть файл: ' + (err.error?.message || err.message)),
    });
  }
}
