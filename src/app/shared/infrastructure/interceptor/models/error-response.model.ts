...
export type ErrorStatus = 0 | 400 | 401 | 403 | 404 | 409 | 422 | 429 | 500 | 502 | 503 | 504;
...
export const errorRecommendations: Record<ErrorStatus, string> = {
  ...
  429: 'Подождите некоторое время перед повторной отправкой запроса.',
...
export function toErrorStatus(status: number): ErrorStatus {
  const validStatuses: ErrorStatus[] = [0, 400, 401, 403, 404, 409, 422, 429, 500, 502, 503, 504];
