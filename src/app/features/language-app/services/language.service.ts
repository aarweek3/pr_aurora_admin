...
export class LanguageService {
  ...
  init(): void {
    ...
    // 1. Проверка общего статуса приложения (Агрегатор и т.д.)
    this.apiService.getAppStatus().subscribe({
      next: (status) => {
        if (!status.hasAggregatorLanguages) {
          console.warn('[LanguageService] ЯЗЫКИ АГРЕГАТОРА НЕ ЗАПОЛНЕНЫ!');
          this.showAggregatorWarning();
        }
      },
      error: (err) => console.error('[LanguageService] Ошибка при проверке статуса:', err),
    });

    // 2. Загрузка языков приложения
    this.apiService
      .getAvailable()
      .pipe(
        tap((languages) => {
  ...
  /**
   * Вывод предупреждения об отсутствии языков Агрегатора
   */
  private showAggregatorWarning(): void {
    this.modal.warning({
      nzTitle: 'Внимание: Языки Агрегатора не настроены!',
      nzContent: `Для корректной работы справочников Агрегатора необходимо инициализировать языки.
                  <br><br>
                  Пройдите в меню: <b>АГРЕГАТОР -> Справочники -> Языки -> Инициализировать</b>.
                  <br><br>
                  Без этого "Seed" данных (типы лицензий и др.) будет выдавать ошибку базы данных.`,
      nzOkText: 'Понятно',
